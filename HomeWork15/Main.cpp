...
// Поскольку вывод матрицы в задаче будет два раза, то, чтоботказатьсяы не повторять один и тот же код, пишем подпрограмму для вывода матрицы в виде функции array_Output()
void array_Output() {
